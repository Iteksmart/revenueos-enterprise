...
import { isSyncProvider, syncIntegrationContacts, type SyncProvider } from "@/lib/integration-sync";
...
export async function POST(request: Request) {
  try {
    ...
    const organizationId = body.organizationId ?? await findLatestOrganizationId();
    const providers = normalizeProviders(body.provider, body.providers);
    const results = [];

    for (const provider of providers) {
      const connectionId = await ensureConnection(organizationId, provider);
      try {
        const result = await syncIntegrationContacts({ organizationId, provider, limit: body.limit });
    ...
    return jsonOk({ organizationId, results });
  } catch (error) {
...
function normalizeProviders(provider?: string, providers?: string[]) {
  const requested = providers?.length ? providers : provider ? [provider] : ["apollo", "notion", "gohighlevel", "hubspot"];
